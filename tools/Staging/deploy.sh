...
dotnet publish SchedyAuthenticator.sln -c release -o $basedir/dist
...
aws cloudformation package \
  ...
  --s3-bucket schedy-deploy-folder \
  --s3-prefix cloudformation/schedy-authenticator/$lowercaseEnvironment

aws cloudformation deploy \
  --stack-name schedy-authenticator-$lowercaseEnvironment \
