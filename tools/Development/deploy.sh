...
dotnet publish PasswordlessDemo.sln -c release -o $basedir/dist
...
aws cloudformation package \
  ...
  --s3-bucket passwordless-demo-deploy-folder \
  --s3-prefix cloudformation/passwordless-demo/$lowercaseEnvironment

aws cloudformation deploy \
  --stack-name passwordless-demo-$lowercaseEnvironment \
